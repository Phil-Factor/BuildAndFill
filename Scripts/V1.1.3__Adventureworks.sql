...
IF @@ERROR <> 0 SET NOEXEC ON
GO
CREATE TYPE [dbo].[DivideBy] AS TABLE
(
[Dividend] [numeric] (8, 2) NOT NULL,
[Divisor] [numeric] (8, 2) NOT NULL
)
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO
...
IF @@ERROR <> 0 SET NOEXEC ON
GO
PRINT N'Creating index [IX_cunningDuplicate] on [Person].[Address]'
GO
CREATE UNIQUE NONCLUSTERED INDEX [IX_cunningDuplicate] ON [Person].[Address] ([AddressLine1], [AddressLine2], [City], [StateProvinceID], [PostalCode])
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO
...
GO
ALTER TABLE [Production].[Document] ADD UNIQUE NONCLUSTERED  ([rowguid])
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO
...
IF @@ERROR <> 0 SET NOEXEC ON
GO
PRINT N'Creating [dbo].[vTopTenSalespeople]'
GO
...
GO
ALTER TABLE [HumanResources].[Employee] WITH NOCHECK ADD CONSTRAINT [CK_Employee_MaritalStatus] CHECK ((upper([MaritalStatus])='S' OR upper([MaritalStatus])='M'))
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO
...
GO
ALTER TABLE [Production].[Product] WITH NOCHECK ADD CONSTRAINT [CK_Product_ProductLine] CHECK ((upper([ProductLine])='R' OR upper([ProductLine])='M' OR upper([ProductLine])='T' OR upper([ProductLine])='S' OR [ProductLine] IS NULL))
GO
IF @@ERROR <> 0 SET NOEXEC ON
GO
...
IF @@ERROR <> 0 SET NOEXEC ON
GO
PRINT N'Disabling constraints on [HumanResources].[Employee]'
GO
...
GO
BEGIN TRY
	EXEC sp_addextendedproperty N'Database_Info', N'[{"Name":"Pubs","Version":"2.1.5","Description":"The Pubs (publishing) Database supports a fictitious bookshop.","Modified":"2020-05-12T11:15:35.830","by":"sa"}]', NULL, NULL, NULL, NULL, NULL, NULL
END TRY
BEGIN CATCH
	DECLARE @msg nvarchar(max);
	DECLARE @severity int;
...
GO
BEGIN TRY
	EXEC sp_addextendedproperty N'SQL_Clone', N'[{"Name":"MaskedProduction","MachineName":"AGENTMACHINE01","User":"DOMAIN\\Username","Modified":"2019-02-21T14:17:12.430","DatabaseName":"Production"}]', NULL, NULL, NULL, NULL, NULL, NULL
END TRY
BEGIN CATCH
	DECLARE @msg nvarchar(max);
	DECLARE @severity int;
