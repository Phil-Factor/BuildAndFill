...
/* important to declare your destination for the data. If you want to add a directory with
the name of the source database (the data donor) then add the macro <The system obligingly tries
to create it if it doesn't exist so with great power comes a certain level of 
responsibility: Well, it should do. */
...
Declare @SourceDatabase sysname=Db_Name();--the project name of the database e.g. Pubs
--automatically substitute the cuurent database name
Select @DataFolderLocation  = replace(@DataFolderLocation,'<dbname>',@SourceDatabase+'\')
...
execute xp_cmdshell ''bcp '+@CurrentDatabase+'.? OUT '+@DataFolderLocation+'\V'+@version+'\'+@SourceDatabase+'-?.bcp -T -N''
	end'
--Select @command
